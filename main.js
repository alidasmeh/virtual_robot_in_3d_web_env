...
const createScene = (canvas) => {
  ...
  const loadBrain = async () => {
    try {
      ...
      if (response.ok) {
        ...
        document.getElementById("brain-indicator").innerText =
          "Pre-trained Brain Loaded";
  ...
  // Native Ground (Ensures stable collisions compared to loaded thin meshes)
  ...
  ground.ellipsoidOffset = new BABYLON.Vector3(0, 0, 0);
  ground.isPickable = true;

  // Make it a bit more visible if needed (Greenish tint)
  const groundMat = new BABYLON.StandardMaterial("groundMat", scene);
  groundMat.diffuseColor = new BABYLON.Color3(0.99, 0.99, 0.99); // Dark greenish
  ...
  BABYLON.SceneLoader.ImportMeshAsync(
    "",
    "./3d_model_files/",
    "3d room without cieling-from blender.glb",
    scene,
  )
    .then((result) => {
      ...
      if (result.meshes.length > 0) {
        ...
        root.scaling.scaleInPlace(0.06);

        // Find the lowest point of the room model and use it to align with y=0 (the greenGround)
      ...
      result.meshes.forEach((mesh) => {
        ...
        // Prevent any error by ensuring ellipsoid properties are initialized
  ...
  faceColors[0] = new BABYLON.Color4(0.0, 0.0, 255, 0.3); // Front Face Blue (RGBA)

  const robotWidth = 0.8;
  const robotHeight = 1.0;
  const robotDepth = 0.6;

  const robot = BABYLON.MeshBuilder.CreateBox(
    "robot",
    {
      width: robotWidth,
      height: robotHeight,
  ...
  robot.ellipsoidOffset = new BABYLON.Vector3(0, 0.05, 0);
  ...
  const faceScreen = BABYLON.MeshBuilder.CreatePlane(
    "faceScreen",
  ...
  const drawFace = (expression = "happy", blink = false) => {
    ...
    if (blink) {
      ...
      faceContext.moveTo(60, 100);
      faceContext.lineTo(100, 100);
      faceContext.moveTo(156, 100);
      faceContext.lineTo(196, 100);
      faceContext.stroke();
    } else {
      // Round eyes
      faceContext.beginPath();
  ...
  const antennaBase = BABYLON.MeshBuilder.CreateCylinder(
    "antennaBase",
    { diameter: 0.05, height: 0.2 },
    scene,
  );
  ...
  const antennaTip = BABYLON.MeshBuilder.CreateSphere(
    "antennaTip",
    { diameter: 0.08 },
    scene,
  );
  ...
  scene.onBeforeRenderObservable.add(() => {
    ...
    const currentExp =
      brainMode === "mapping" || brainMode === "auto-nav"
        ? "thinking"
        : "happy";
  ...
  const showSampledPoints = (scene, brain) => {
    ...
    sampledPointsContainer = new BABYLON.TransformNode(
      "sampledPointsContainer",
      scene,
    );
    ...
    const buttonMesh = BABYLON.MeshBuilder.CreateCylinder(
      "sampledButton",
      {
        diameter: 0.3,
        height: 0.05,
    ...
    buttonMesh.ellipsoid = new BABYLON.Vector3(0.5, 0.5, 0.5);
    buttonMesh.ellipsoidOffset = new BABYLON.Vector3(0, 0, 0);

    const matricesData = new Float32Array(
      grid.filter((val) => val === 1).length * 16,
    );
    let count = 0;

    for (let i = 0; i < grid.length; i++) {
      if (grid[i] === 1) {
        // Only show traversable points
  ...
  async function populateModelList() {
    ...
    try {
      ...
      if (models.length === 0) {
        modelList.innerHTML =
          '<p class="empty-msg">No models found in folder "trained_models".</p>';
        return;
      }
  ...
  scene.onBeforeRenderObservable.add(() => {
    ...
    if (brainMode === "manual") {
      ...
      if (inputMap["ArrowDown"]) {
        robot.moveWithCollisions(backward);
        moved = true;
      }
      ...
      if (currentTarget) {
        ...
        if (reached) {
          ...
          brainIndicator.className = "value active";
          setTimeout(() => {
            brainIndicator.innerText = "Idle Brain";
            brainIndicator.className = "value idle";
          }, 3000);
        } else {
  ...
  scene.gravity = new BABYLON.Vector3(0, -0.02, 0);
